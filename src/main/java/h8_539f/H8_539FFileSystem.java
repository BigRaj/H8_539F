...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = H8_539FFileSystem.MyFileSystemFactory.class)
public class H8_539FFileSystem implements GFileSystem {
	 ...
	 */
	public H8_539FFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + H8_539FFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<H8_539FFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public H8_539FFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			H8_539FFileSystem fs = new H8_539FFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
